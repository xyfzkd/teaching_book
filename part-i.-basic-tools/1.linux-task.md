...
1） 下载方法1： 在自己的机器系统（host: 如Mac, Windows）上操作：

* yeast基因组注释文件可以从浏览器下载：​[Download Link](.gitbook/files/1.gtf.gz)

* 然后把这个文件考到host的共享目录中： `~/Desktop/bioinfo_tsinghua_share`


2） 下载方法2： 在docker 容器(container) 里的Linux系统操作：

* 首先要运行docker开启Linux的容器：

`docker exec -it bioinfo_tsinghua bash`

> 在自己电脑（host）的 Terminal 中运行docker容器，详情请参见 Getting Started
...
> 注意：如果挂载主机目录，请注意切换工作目录 `/home/test/share`。
