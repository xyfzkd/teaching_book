...
1. 阅读 **4a) read more** 中给出的 rMATS 的文献（<https://doi.org/10.1073/pnas.1419161111>），简要阐述 rMATS 找到 differential alternative splicing 的原理（只需解释 Unpaired Replicates 的情形即可）。
1. 为了鉴定 CUGBP1 对 mRNA isoform 的调控，科学家在 C2C12 小鼠成肌细胞（myoblast）中分别表达空载体（`SRR065546.bam`）和含有干扰 CUGBP1 的 shRNA 的载体（`SRR065547.bam`）。请同学们至 [这里](https://cloud.tsinghua.edu.cn/d/232942b5c5ad41fb8881/) 下载输入文件，探索在 X 染色体上存在 differential alternative splicing 的基因。（需要上交代码和输出结果中所有以 `.MATS.JCEC.txt` 结尾的文件）
