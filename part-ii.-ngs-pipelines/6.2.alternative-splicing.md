...
1. `A3SS`: alternative 3' splice site 
1. `SE`: skipped exon
1. `MXE`: mutually exclusive exons
1. `RI`: retained intron
...
1. now your working directory looks like this
...
1. (optional) 阅读 **4a) read more** 中给出的 rMATS 的文献（<https://doi.org/10.1073/pnas.1419161111>），简要阐述 rMATS 找到 differential alternative splicing 的原理（只需解释 Unpaired Replicates 的情形即可）。
...
cd /home/test/alter-spl
mkdir input2
wget -P input2 https://raw.githubusercontent.com/dongzhuoer/lulab-teaching_book-data/master/alter-spl/SRR065546_chrX.bam
wget -P input2 https://raw.githubusercontent.com/dongzhuoer/lulab-teaching_book-data/master/alter-spl/SRR065547_chrX.bam

samtools view input2/SRR065546_chrX.bam | cut -f 10 | perl -ne 'chomp;print length($_) . "\n"' | sort | uniq -c
samtools view input2/SRR065547_chrX.bam | cut -f 10 | perl -ne 'chomp;print length($_) . "\n"' | sort | uniq -c

echo "input2/SRR065546_chrX.bam" > input2/b1.txt
echo "input2/SRR065547_chrX.bam" > input2/b2.txt

python2 /usr/local/rMATS-turbo-Linux-UCS4/rmats.py \
    --b1 input2/b1.txt --b2 input2/b2.txt --gtf input/Mus_musculus_chrX.gtf --od output2 \
    -t paired --readLength 35

ls output2 -hl
cat output2/A5SS.MATS.JCEC.txt
```
