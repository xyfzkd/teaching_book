...
![](../.gitbook/assets/chip-seq-pipeline.jpeg)
...
1. download example data from [here](https://github.com/dongzhuoer/lulab-teaching_book-data/tree/master/chip-seq/input)
...
docker exec chip-seq ls ../bin/makeTagDirectory

docker exec chip-seq ../bin/makeTagDirectory input/ip    input/ip.part.bam
docker exec chip-seq ../bin/makeTagDirectory input/input input/input.part.bam
docker exec chip-seq mkdir output
docker exec chip-seq ../bin/findPeaks input/ip -style factor -o output/part.peak -i input/input/
docker exec chip-seq head output/part.peak -n 100

docker exec chip-seq ../bin/findMotifsGenome.pl output/part.peak sacCer2 output/part.motif.output -len 8
docker exec chip-seq ls output/part.peak
docker exec chip-seq ls output/part.motif.output


docker exec chip-seq macs14 -t input/ip.part.bam -c input/input.part.bam --name=yeast_macs_p05  --format=BAM --gsize=1.2e7  --tsize=50 --pvalue=1e-5 --mfold=10,30
docker exec chip-seq cat yeast_macs_p05_peaks.xls
docker exec chip-seq cat yeast_macs_p05_negative_peaks.xls
```
...
```bash
macs2 callpeak -t ip.part.bam -c input.part.bam --name=yeast_macs_p05  --format=BAM --gsize=1.2e7  --tsize=50 --pvalue=1e-5
```

The main output is `yeast_macs_p05_peaks.xls`, which is a tabular file containing information about called peaks. You can open it in excel and sort/filter using excel functions. Information include:
...
1. 根据 [这里](https://github.com/dongzhuoer/lulab-teaching_book-data/tree/master/chip-seq/homework) 的 `input.chrom_part.bam` 和 `ip.chrom_part.bam`）寻找 peak 和 motif。
