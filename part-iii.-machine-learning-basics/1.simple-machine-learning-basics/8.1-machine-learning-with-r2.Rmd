...
we need to install the following packages:
  ...
  1. `mlbench`: machine learning datasets

```{r install-internal, include=FALSE}
for(pkg in c('dplyr', 'randomForest', 'ROCR', 'GGally', 'mlbench')){
...
m <- as.matrix(df[,colnames(df) != "Species"])
...
# generate a random permutation from 1 to number of samples
indices = sample(1:nrow(df))
# select the first 80% samples as training set
training <- df[indices[1:nrow_traing,]
# select the remaining 20% samples as test set
testing <- df[indices[(n_train + 1):nrow(df)]]
```
